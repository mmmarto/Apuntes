...
1. **Subrayar el enunciado.** Sustantivos importantes → clases o atributos. Verbos → métodos. Ej: "una _cita_ tiene una _fecha_" → `Cita` es clase, `fecha` es atributo.
...
> [!note] Getters y setters Puse los que tienen sentido en cada clase. Si la cátedra los pide todos, va un get y un set por atributo.
...
classDiagram
    class CuentaBancaria {
        ...
        -fechaCreacion : Date
        +CuentaBancaria(t : String, max : int)
...
> - Agregué `extraccionesRealizadas`: si hay un _máximo_ de extracciones, la cuenta tiene que ir contando cuántas lleva para saber cuándo frenar.
> - `extraer` devuelve `boolean` para avisar si pudo o no (sin saldo o pasado del máximo → `false`).
> - El constructor no recibe la fecha: adentro hace `fechaCreacion = new Date()`.
...
> - **Composición (◆)**: ese punto es el centro de _ese_ círculo; sin el círculo no tiene sentido. Una asociación simple también se suele aceptar.
> - `area()` devuelve π·r².
...
> [!note] Cómo lo pensé La pregunta clave: **¿la parte puede existir o usarse sin el todo?**
> 
> - **Rueda y Motor → agregación (◇):** una rueda se pasa a otro auto, un motor se cambia. Existen solos.
> - **Chasis y Puerta → composición (◆):** el número de chasis _es_ la identidad del auto y la puerta está hecha para esa carrocería.
...
classDiagram
    class Cita {
        -fecha : Date
        -importancia : int
        +Cita(f : Date, c : Persona, l : Lugar, i : int)
        +getFecha() Date
        +getContacto() Persona
        +getLugar() Lugar
        +getImportancia() int
    ...
    class Persona {
        ...
        -telefono : String
        -email : String
        +Persona(n : String, a : String, d : String)
        +getNombre() String
        +getDni() String
    }
    class Lugar {
        ...
        -direccion : String
        -ciudad : String
        +Lugar(n : String, d : String, c : String)
        +getNombre() String
    }
...
classDiagram
    class Guitarra {
        ...
        +Guitarra(n : String, p : float, f : Date, e : EspecificacionGuitarra)
        +getPrecio() float
...
> [!note] a) ¿Cómo se hace? Separo lo **propio de cada guitarra física** de lo que **describe al modelo**.
> 
...
classDiagram
    class Tarea {
        ...
        -prioridad : int
        +Tarea(d : String, f : Date, p : int, r : Persona)
    ...
    class Persona {
        ...
        -email : String
        -telefono : String
        +Persona(n : String, d : String, dom : String, e : String, t : String)
...
classDiagram
    class Visita {
        -fecha : Date
        +Visita(f : Date, v : Persona, m : Museo, o : ObraDeArte)
        +getFecha() Date
    ...
    class Museo {
        ...
        -direccion : String
        -ciudad : String
        +Museo(n : String, d : String, c : String, dir : Persona)
        +getNombre() String
        +getDirector() Persona
    ...
    class ObraDeArte {
        ...
        -anioCreacion : int
        -tipo : String
        +ObraDeArte(t : String, a : Persona, anio : int, tipo : String)
        +getTitulo() String
        +getAutor() Persona
    }
    class Persona {
...
> [!note] Cómo lo pensé Idea nueva: **la misma clase puede cumplir distintos roles.**
> 
